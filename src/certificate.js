...
var dob="09/11/1995"
var pob="Lyon"
var adresse="2 Rue Saint Exupery"
var ville="Meudon la fôret"
var cp="92360"
...
async function generatePdf (profile, reasons) {
  ...
  const data = [
    `Cree le: ${creationDate} a ${nouvelleHeure}`,
    `Nom: ${lastname}`,
    `Prenom: ${firstname}`,
  ...
  drawText(`${creationDate} à ${nouvelleHeure}`, 455, 144, 7)
...
$('#generate-btn').addEventListener('click', async event => {
  ...
  $('#field-firstname').value=prenom;
  $('#field-lastname').value=nom;
  $('#field-birthday').value=dob;
  $('#field-lieunaissance').value=pob;
  $('#field-address').value=adresse;
  $('#field-town').value=ville;
  $('#field-zipcode').value=cp;
  $('#checkbox-sport').checked=true;
  setReleaseDateTime()
  var minute=$('#field-heuresortie').value.substring(3)
  var heure=$('#field-heuresortie').value.substring(0,2)
  minute=minute-20
  if (minute<0){
    heure--;
    minute=60+minute;
  }
  nouvelleHeure=heure+":"+minute;
  $('#field-heuresortie').value=heure+":"+minute;
  saveProfile()
  ...
  downloadBlob(pdfBlob, `attestation-${creationDate}_${nouvelleHeure}.pdf`) 
